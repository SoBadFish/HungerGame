...
import org.sobadfish.hunger.room.GameRoomCreater;
...
import org.sobadfish.hunger.top.TopItem;
...
public class HungerGameAdminCommand extends Command {
    ...
    public boolean execute(CommandSender commandSender, String s, String[] strings) {
        ...
        if (strings.length > 0 && "help".equalsIgnoreCase(strings[0])) {
            commandSender.sendMessage("只需要输入/hga 就可以了");
            commandSender.sendMessage("其他指令介绍:");
            commandSender.sendMessage("/hga reload 重新载入配置");
            commandSender.sendMessage("/hga set [名称] 创建一个自定义房间模板");
            commandSender.sendMessage("/hga tsl 读取模板的队伍数据");
            commandSender.sendMessage("/hga see 查看所有加载的房间");
            commandSender.sendMessage("/hga close [名称] 关闭房间");
            commandSender.sendMessage("/hga exp [玩家] [数量] <由来> 增加玩家经验");
            commandSender.sendMessage("/hga status 查看线程状态");
            commandSender.sendMessage("/hga end 停止模板预设");
            commandSender.sendMessage("/hga float add/remove [房间名称] [名称] [文本] 在脚下设置浮空字/删除浮空字");
            commandSender.sendMessage("/hga cancel 终止房间创建");
            commandSender.sendMessage("/hga top add/remove [名称] [类型] [房间(可不填)] 创建/删除排行榜");
