...
public class GameRoomCreater {
    ...
    public void onCreatePreset(String value){
        ...
        switch (setFlag){
            case 1:
                ...
                creater.sendForceMessage("&e继续执行 &r/hga set &a[最低玩家数]&e 执行下一步操作");
                roomName = value;
                setFlag++;
                break;
            case 2:
                creater.sendForceMessage("&2设置最低人数 "+value);
                creater.sendForceMessage("&e继续执行 &r/hga set &2[出生点数量]&e 执行下一步操作");
                min = Integer.parseInt(value);
                setFlag++;
                break;
            case 3:
                creater.sendForceMessage("&2设置每个队伍的出生点数量 "+value);
                creater.sendForceMessage("&e继续执行 &r/hga set &2[最大玩家数]&e 执行下一步操作");
        ...
        switch (flag) {
            case 1:
                if(roomConfig == null) {
                    ...
                    creater.sendForceMessage("继续执行/hga 进行下一步 [进入游戏地图设置]");
                }else{
                    creater.sendForceMessage("&2成功预设房间设置");
                    creater.sendForceMessage("&e继续执行 &r/hga &r进行下一步 &b[进入游戏地图设置]");
                }
                flag++;
                break;
            case 2:
                ...
                creater.sendForceMessage("&e继续执行 &r/hga &e进行下一步 &b[设置等待大厅]");
                flag++;
                break;
            case 3:
                worldInfoConfig.setWaitPosition(creater.getPosition());
                creater.sendForceMessage("&2成功等待大厅");
                creater.sendForceMessage("&e继续执行 &r/hga &e进行下一步 &r[&b设置"+(new ArrayList<>(roomConfig.teamCfg.keySet()).get(team.size()))+"出生点 &21&b /&d "+spawnSize+"&r]");
                flag++;
                break;
            case 4:
    ...
    private boolean createSpawnPos(){
        ...
        if(positions.size() < spawnSize){
            ...
            if(positions.size() != spawnSize) {
                creater.sendForceMessage("&e继续执行 &r/hga &e进行下一步 &r[&b设置队伍出生点&r " + teamName + " [&2" + (positions.size() + 1) + " &b/&d "+spawnSize+"&r]");
            }else{
                if(positions.size() > spawnFlag+1){
                    creater.sendForceMessage("&2设置 &r" + teamName + " &2出生点坐标完成");
                    creater.sendForceMessage("&e继续执行 &r/hga &e进行下一步 &r[&b设置 &r"+ new ArrayList<>(roomConfig.teamCfg.keySet()).get(spawnFlag + 1) + " &2出生点 &r [&21 &b/&d "+spawnSize+"&r]");
                }
                spawnFlag++;
